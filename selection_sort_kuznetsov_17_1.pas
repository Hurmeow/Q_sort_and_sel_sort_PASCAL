...
procedure sel_sort(len, mode : integer; var data_sort : data;
                                  var step, trade : integer);
{Procedura prostoy sortirovki,
na vhod podaem dlinu posled., rezhim raboty;
na vihode poluchaem otsortirovanuy posledovatelnost,
chislo sravneniy i perestanovok}

    var i, j, k, indMin, valMin : integer;
        listMin : list;
    ...
    begin
        ...
        for i := 1 to (len - 1) do begin
                ...
                listMin := data_sort[i];
                valMin :=  data_sort[i].h;
                indMin := i;

                for j := i + 1 to len do begin
                        inc(step);
                        if (data_sort[j].h < valMin) then begin
                                listMin := data_sort[j];
                                valMin := data_sort[j].h;
                                indMin := j;
                        end;

                end;

                if (i <> indMin) then begin
                        data_sort[indMin] := data_sort[i];
                        data_sort[i] := listMin;
                        data_sort[i].h := valMin;
                        inc(trade);
                end;
        end;
        if (mode = 1) then begin
                writeln();
                writeln('Sorting step: ', len:3);
...
procedure table_write(f_write, rlt_write : string);
    ...
    begin
        ...
        writeln(file_w, 'The sort table is <Selection sort>:');
        ...
        for num := 1 to len_seq do begin
                ...
                for v_file := 1 to var_file do begin
                        ...
                        sel_sort(seq, mode, arr_d, step, trade);
...
begin
        ...
        f_write := 'f_n_up_select_sort.txt';
        rlt_write := 'Result_sel_sort.txt';
        ...
        if rlt_mode = 1 then table_write(f_write, rlt_write)
                ...
                sel_sort(n, mode, arr_d, step, trade);
